  ...
  return (
    <div>
      ...
      <p className="mt-1 text-ink-500">
        Stays you’ve saved for later — kept on this device
      </p>
      ...
      {items.length === 0 ? (
        <div className="mt-10 rounded-md border border-line bg-sand/40 p-10 text-center">
          ...
          <p className="mx-auto mt-2 max-w-sm text-sm text-ink-500">
            Tap the heart on a hotel to save it here. Your shortlist persists across refresh.
          </p>
        ...
        <ul className="mt-8 space-y-4">
          {items.map((item) => (
            <li
              key={item.id}
              className="flex flex-col gap-4 rounded-md border border-line bg-paper p-4 sm:flex-row sm:items-center"
            >
