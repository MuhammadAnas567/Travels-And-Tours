...
export default async function DashboardPage() {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-ink-700">Manage your bookings and profile</p>

      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        <Card>
          <CardContent className="p-6">
            <p className="text-sm text-ink-500">Total Bookings</p>
            <p className="text-3xl font-bold text-primary-700">{bookings.length}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-6">
            <p className="text-sm text-ink-500">Upcoming Trips</p>
            <p className="text-3xl font-bold text-primary-700">{upcoming.length}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-6">
      ...
      <Card className="mt-8">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Link href="/dashboard/bookings" className="text-sm text-primary-500 hover:underline">
            View all
          </Link>
        </CardHeader>
        <CardContent>
          {bookings.length === 0 ? (
            <p className="text-ink-500">No bookings yet. <Link href="/tours" className="text-primary-500">Browse tours</Link></p>
          ) : (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {booking.status === "CONFIRMED" && (
                      <Link href={`/dashboard/bookings/${booking.id}/ticket`}>
                        <Ticket className="h-5 w-5 text-primary-500" aria-label="View e-ticket" />
                      </Link>
                    )}
                  </div>
                </div>
